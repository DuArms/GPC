...
std::vector<Point> poly_points(Point center, double radius) {
    const int n = 6;
    const double Pi = 3.14159265358979323846;
    std::vector<Point> corners;
    float starting_angle = 2 * Pi / n;

    for (int i = 0; i < n; i++) {
        double alpha = (i) * starting_angle;
        Point new_point = Point(center.x + radius * cos(alpha), center.y + radius * sin(alpha));

        corners.push_back(new_point);
    }
...
void draw_hexagon(const std::vector<Point>& corners, int mode = 1) {
    if (mode == 0) glPolygonMode(GL_FRONT, GL_LINE);
...
void Display8() {
    ...
    auto corners_blue = poly_points(Point(0, 0), 0.6);

    auto corners_red = poly_points(Point(0, 0), 0.3);

    auto corners_white = poly_points(Point(0, 0), 0.3);
    glColor3f(0.1, 0.1, 1);
    draw_hexagon(corners_blue,1);

    glColor3f(1, 1, 1);
    draw_hexagon(corners_white,1);

    glColor3f(1, 0.1, 0.1);
    draw_hexagon(corners_red,0);
